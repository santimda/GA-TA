...
class R():
	...
	def __init__(self, Data):
		...
		if not Data:
			raise ValueError('you must to espicify where the Data is.')
		...
		for each in Data.markers:
			self.marker_mod.append(str(each)+'A')
			self.marker_mod.append(str(each)+'B')
		
		#self.marker_mod = np.asarray(self.marker_mod)

		#print '======================================='
		#print 'data.populations shape', np.shape(data.populations)
		#print data.populations[3]

		self.womens, self.mens = self.R_Type(Data)
		
	def R_Type(self, Data):
		...
		poblacion = Data.populations[0] # A ser modificado (ver nota)
		poblacion_w = []
		poblacion_m = []
